...
Escenario más exigente que sigue siendo plausible: riesgo y saldos consultados a un servicio o BD **en cada orden**. Consume el 63 % del presupuesto de 12,4 ms.
...
Una salvedad vigente. La cláusula «sin exigir más de un núcleo por partición» de H2 **quedó respaldada**: medida durante F2, cada partición usó 23,6 % de un núcleo en media y 58,3 % como máximo. Y el resultado tiene fondo — a 42 órd/s con 8 ms por orden el trabajo es 33,6 % de un núcleo, que extrapolado a las 125 órd/s del techo `1/S` da exactamente 100 %: **el techo de throughput y el límite de un núcleo son el mismo hecho**, consecuencia estructural del único escritor. Lo que sigue abierto es el **p99.9, que excede los 200 ms en el pico** (231 ms en F2+F3, 352 ms en F4): el contrato es sobre p95 y se cumple, pero si se endureciera a p99, S = 8 ms no alcanzaría. Esa cifra es además un **piso**: el A/B de forma muestra que con una distribución de servicio sin cota superior el mismo escenario empeora un 23 %.
...
El contraste entre esas dos cifras es en sí un resultado sobre H2: repartir el pico entre dos particiones sube el presupuesto **1,46×**, no el 2× que sugiere repartir la carga a la mitad. **No es una pérdida de eficiencia sino aritmética del SLA**: shardear baja ρ y con ello la espera, pero el tiempo de servicio *también es latencia*, y subir el presupuesto lo sube directamente. Con N=2 y S = 17 ms tendríamos el mismo ρ que el caso caliente, pero el doble de latencia media. Verificado: los dos montajes caen sobre la misma curva de p95/media contra ρ.
...
3. **Hallar el N mínimo de shards, no solo validar el N elegido** *(recomendación general más fuerte de la sesión)* — ✅ **resuelto, y la respuesta es condicional**. El techo por shard no es una propiedad del patrón sino `1/S`, y la Serie A lo midió con un `match()` de ~13 µs, de donde salía la cifra engañosa de «> 1.000 órdenes/s». Medido con el punto de operación declarado, **el techo es 125 órd/s** y el pico contractual ocupa el **67 % de una sola partición**. F4 de la Serie B *es* funcionalmente la corrida N=1 —todo el tráfico cae en un shard— y pasó con p95 = 148,09 ms.
   ...
   La conclusión de escalabilidad queda así: **N = 1 basta para el contrato si el costo por orden se mantiene bajo ~8,5 ms**; N = 2 sube ese límite a 12,4 ms y da margen. N no se elige por el patrón sino dimensionando contra el costo real de la lógica de negocio, que el PoC no implementa. La corrida `make f2-n1` deja de ser necesaria para esta conclusión.
